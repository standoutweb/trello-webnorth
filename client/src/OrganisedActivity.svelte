<script>
	...
	onMount( async () => {
		let before = '2024-06-25T00:00:00Z';
		let after = '2024-06-24T00:00:00Z';
	...
	<div class="project">
		...
		<ul>
			{#each cards as { cardName, actions, date, shortLink }}
				<li>
					<a href={`https://trello.com/c/${shortLink}`} target="_blank">{cardName}
						- {`https://trello.com/c/${ shortLink }`}</a>
					<small style="user-select: none;">{actions} {date}</small>
				</li>
			{/each}
		</ul>
	</div>
